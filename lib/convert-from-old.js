...
module.exports = documents => {
  return new Promise((resolve) => {
    const converted = documents.map(document => {
      try {
        logger('info', ['convert-from-old', document._id])
        ...
        logger('info', ['convert-from-old', document._id, type, 'added specific properties'])
        saveFile({ filePath: `${__dirname}/../test/new/${document._id}.json`, data: newDocument })
        return newDocument
      } catch (error) {
        logger('error', ['convert-from-old', document._id, document.documentType, document.documentCategory, error])
      }
    })
